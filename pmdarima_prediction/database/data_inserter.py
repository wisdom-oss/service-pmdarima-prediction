...
def create_table() -> None:
    ...
    table_string = (
        f"CREATE TABLE IF NOT EXISTS timeseries.water_demand_prediction (date TIMESTAMPTZ NOT NULL, "
...
def create_hypertable() -> None:
    ...
    with db_connector.create_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT create_hypertable('timeseries.water_demand_prediction', by_range('date'));"
            )
