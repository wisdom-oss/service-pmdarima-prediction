...
def create_forecast_data(
    model: ARIMA, n_periods: int, exogenous_df: pd.DataFrame | None
) -> pd.DataFrame:
    """
    ...
    df = pd.DataFrame(
        {
            "lowerConfValues": conf_intervals[:, 0],
...
def create_forecast_labels(
    last_timestamp: str, n_periods: int, resolution: str
