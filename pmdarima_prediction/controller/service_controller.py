...
def get_smartmeter_data(
    meter_name: str, timeframe: str, resolution: str, start_date: str
) -> interfaces.SmartmeterData | None:
    """
...
def create_end_date(
    timeframe: str, start_point: datetime.datetime
) -> datetime.datetime:
    """
...
def train_model(
    ...
    if not model_handling.model_is_unique(
        meter_name,
        timeframe,
        resolution,
        start_date_string,
    ...
    end_date = create_end_date(timeframe, start_date).replace(
    ...
    if weather_capability == "plain":
        ...
        weather_df = dwd_weather.get_weather_data(
            weather_capability, column_name, start, end
        )
        model, train_time = model_training.train_model(
            df[["value"]], weather_df[[column_name]]
        )
    ...
    model_handling.save_model_by_name(
        model_dict,
        meter_name,
        timeframe,
        resolution,
    ...
    return {
        ...
        "details": {
            ...
            "modelStartDate": start_date.isoformat(),
            "modelEndDate": end_date.isoformat(),
        },
    }
...
def forecast(
    ...
    model_dict = model_handling.load_model_by_name(
        meter_name, timeframe, resolution, start_date, weather_capability, column_name
    )

    # create 24 forecast label dates
    forecast_labels = data_forecast.create_forecast_labels(
        model_dict["end_date"], 23, resolution
    )
    ...
    if weather_capability == "plain":
        ...
        weather_df = dwd_weather.get_weather_data(
            weather_capability,
            column_name,
            int(forecast_labels[0].timestamp()),
            int(forecast_labels[-1].timestamp()),
        )

        print(weather_df.tail(5))

        forecast_df = data_forecast.create_forecast_data(
            model_dict["model"], 23, weather_df[[column_name]]
        )

    # select real values to compare with predicted data
    real_values = ds.select_date_value(
        meter_name, forecast_labels[0], forecast_labels[-1]
    )["value"]

    # evaluate model prediction
    metrics_df = model_metrics.calculate_metrics(
        real_values, forecast_df["value"][:22]
    )  # FIXME: Forecast Array is too long by one
...
def __resample_data(
    data: interfaces.SelectDateValueData, resolution: str
) -> interfaces.SelectDateValueData:
    """
